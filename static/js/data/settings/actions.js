...
export const setNotificationSound = (notificationSound) => ({
  type: 'SET_NOTIFICATION_SOUND',
  notificationSound,
})

export const startSetNotificationSound = (notificationSound) => {
  return async (dispatch, getState) => {
    const uid = getState().auth.uid

    dispatch(setNotificationSound(notificationSound))

    await fs
      .doc(`users/${uid}`)
      .set({ settings: { notificationSound } }, { merge: true })
  }
}
